...
import Templates from "../components/Simulation/Templates";
...
export default function Manager(props) {
  ...
  const [metaData, setmetaData] = React.useState(
    JSON.parse(window.localStorage.getItem("metaData")).simulation
  );
  ...
  const adminRoutes = {
    Simulator: {
      ...
      component: (
        <Simulator
          permission={getPermission("Simulator")}
    ...
    Templates: {
      ...
      component: (
        <Templates
          permission={getPermission("Templates")}
          history={props.history}
        />
      ),
    },
  ...
  const switchRoutes = (
    <Switch>
      ...
      <Redirect to={`/simulation/${metaData[0].path}`} />
    </Switch>
  );
