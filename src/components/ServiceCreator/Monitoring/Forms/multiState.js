...
export default function multiState(props) {
  ...
  const multiStateForm = useFormik({
    initialValues: {
      description: "",
      datapoint: "",
      states: [
    ...
    validationSchema: Yup.object({
      ...
      states: Yup.array().of(
        Yup.object().shape({
          ...
          state: Yup.boolean().required("Required field"),
        })
      ),
  ...
  const statesEndRef = useRef(null);

  const scrollToBottom = () => {
    statesEndRef.current.scrollIntoView();
  };
  ...
  useEffect(() => {
    if(multiStateForm.values.operation == ""){
      setErrors([])
    }
    const operationArray = multiStateForm.values.operation.split("")
    const noOfOpenParanthesis = multiStateForm.values.operation.split("{").length -1
    const noOfClosedParanthesis = multiStateForm.values.operation.split("}").length -1
  ...
  }, [multiStateForm.values.operation])
  ...
  const checkChar = (char, i, operation) => {
    const operationString = operation ? operation : multiStateForm.values.operation
  ...
  const validateOperation = () => {
    ...
    const operationString = multiStateForm.values.operation.replaceAll(",", ", ").replaceAll("(", "( ")
    ...
    let newString = multiStateForm.values.operation
    ...
    if(!newErrors.length){
      if (newString !== multiStateForm.values.operation){
        multiStateForm.setFieldValue("operation", newString)
  ...
  const selectDatapointFromTheList = (datapoint)=>{
    let newInputValue = multiStateForm.values.operation;
    ...
    multiStateForm.setFieldValue('operation',newInputValue)
  }
  ...
  return (
    <div>
      ...
      <form onSubmit={multiStateForm.handleSubmit}>
        <p
          style={{
            fontSize: "16px",
            color: "#666666",
        ...
        <TextField
          ...
          margin="dense"
          id="name"
          error={multiStateForm.touched.name && multiStateForm.errors.name}
          value={multiStateForm.values.name}
          onChange={multiStateForm.handleChange}
          onBlur={multiStateForm.handleBlur}
          helperText={
            multiStateForm.touched.name ? multiStateForm.errors.name : ""
          }
        />
        {props.type == "datapoint" ? <TextField
          ...
          margin="dense"
          id={props.type}
          error={multiStateForm.touched.datapoint && multiStateForm.errors.datapoint}
          value={multiStateForm.values.datapoint}
          onChange={multiStateForm.handleChange}
          onBlur={multiStateForm.handleBlur}
          helperText={
            multiStateForm.touched.datapoint ? multiStateForm.errors.datapoint : ""
          }
        />: 
        <FormTooltip
        open={infoPopup}
      ...
      >
          <div
            ...
            <TextField
              ...
              margin="dense"
              id={"operation"}
              value={multiStateForm.values.operation}
              error={multiStateForm.touched.operation && multiStateForm.errors.operation}
              ...
              onBlur={multiStateForm.handleBlur}
              helperText={
                multiStateForm.touched.operation ? multiStateForm.errors.operation : ""
              }
            />
            <div style={{position: "absolute", top: "24px", left: "14px", width: "500px", overflowWrap: "break-word", lineHeight: "22px", fontSize: "15px", letterSpacing: "normal", fontFamily: "Arial, Helvetica, sans-serif"}}>
              {multiStateForm.values.operation !== "" ?
                multiStateForm.values.operation.split("").map((char,i, arr) => <span style={{color: checkChar(char, i)}} key={i}>
                        ...
                        <Fragment>
                          ...
                          {char == "{" ? <span style={{ fontWeight: "bold", display: 'flex', justifyContent: 'space-between' }}>Available Datapoints  <span style={{ cursor: 'pointer' }} onClick={() => { setShowHTML_Tooltip(false) }}>
                            ...
                            {props.datapoints.map((data) => {
                                const splitIndex = multiStateForm.values.operation.lastIndexOf("{")
                                if (i == splitIndex){
                                  return (<span style={{ cursor: 'pointer' }} onClick={() => { selectDatapointFromTheList(data) }}>{`${data.friendlyName} (${data.name})`}</span>)
                                }
                                const datapoint = multiStateForm.values.operation.slice(splitIndex+1)
        ...
        <TextField
          ...
          id="description"
          error={
            multiStateForm.touched.description &&
            multiStateForm.errors.description
          }
          value={multiStateForm.values.description}
          onChange={multiStateForm.handleChange}
          onBlur={multiStateForm.handleBlur}
          helperText={
            multiStateForm.touched.description
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                checked={multiStateForm.values.defaultTimeseries}
                onChange={multiStateForm.handleChange}
              />
            }
          />
        ...
        >
          ...
          <FormControlLabel
            control={
              <Switch
                name="config"
                checked={multiStateForm.values.config}
                onChange={multiStateForm.handleChange}
              />
            }
          />
        ...
        <FormikProvider value={multiStateForm}>
          <FieldArray
            name="states"
            render={(arrayHelpers) => (
              <div style={{ marginTop: "20px" }}>
                ...
                >
                  {multiStateForm.values.states.map((elm, index) => (
                    <div style={{ width: "100%", marginBottom: "10px" }}>
                      ...
                      <div style={{ display: "flex", gap: "20px" }}>
                        <TextField
                          ...
                          fullWidth
                          margin="dense"
                          name={`states[${index}].label`}
                          value={multiStateForm.values.states[index].label}
                          onChange={multiStateForm.handleChange}
                          // error={
                          //   multiStateForm.touched.defaultName &&
                          //   multiStateForm.errors.defaultName
                          // }
                          // value={multiStateForm.values.defaultName}
                          // onChange={multiStateForm.handleChange}
                          // onBlur={multiStateForm.handleBlur}
                          // helperText={
                          //   multiStateForm.touched.defaultName
                        ...
                        <TextField
                          ...
                          value={multiStateForm.values.states[index].value}
                          onChange={multiStateForm.handleChange}
                          // error={
                          //   multiStateForm.touched.defaultCommand &&
                          //   multiStateForm.errors.defaultCommand
                          // }
                          // value={multiStateForm.values.defaultCommand}
                          // onChange={multiStateForm.handleChange}
                          // onBlur={multiStateForm.handleBlur}
                          // helperText={
                          //   multiStateForm.touched.defaultCommand
                        ...
                        <FormControlLabel
                          control={
                            <Switch
                              ...
                              checked={
                                multiStateForm.values.states[index].state
                              }
                              onChange={(e) =>
                                multiStateForm.setFieldValue(
                                  `states[${index}].state`,
                                  e.target.checked
                                )
                              }
