...
export default function Reading(props) {
  ...
  useEffect(() => {
    if(readingForm.values.operation == ""){
      setErrors([])
    }
    const operationArray = readingForm.values.operation.split("")
    const noOfOpenParanthesis = readingForm.values.operation.split("{").length -1
    const noOfClosedParanthesis = readingForm.values.operation.split("}").length -1
  ...
  const checkChar = (char, i, operation) => {
    const operationString = operation ? operation : readingForm.values.operation
  ...
  const validateOperation = () => {
    ...
    const operationString = readingForm.values.operation.replaceAll(",", ", ").replaceAll("(", "( ")
    ...
    let newString = readingForm.values.operation
    ...
    if(!newErrors.length){
      if (newString !== readingForm.values.operation){
        readingForm.setFieldValue("operation", newString)
  ...
  const selectDatapointFromTheList = (datapoint)=>{
    let newInputValue = readingForm.values.operation;
    ...
    readingForm.setFieldValue('operation',newInputValue)
  }
  ...
  return (
    <div>
      ...
      <form onSubmit={readingForm.handleSubmit}>
        <p
          style={{
            fontSize: "16px",
            color: "#666666",
        ...
        <TextField
          ...
          margin="dense"
          id="name"
          error={readingForm.touched.name && readingForm.errors.name}
          value={readingForm.values.name}
          onChange={readingForm.handleChange}
          onBlur={readingForm.handleBlur}
          helperText={readingForm.touched.name ? readingForm.errors.name : ""}
        />
        {props.type == "datapoint" ? <TextField
          ...
          margin="dense"
          id={props.type}
          error={readingForm.touched.datapoint && readingForm.errors.datapoint}
          value={readingForm.values.datapoint}
          onChange={readingForm.handleChange}
          onBlur={readingForm.handleBlur}
          helperText={
            readingForm.touched.datapoint ? readingForm.errors.datapoint : ""
          }
        />: 
        <FormTooltip
          open={infoPopup}
        ...
        >
          <div
            ...
            <TextField
              ...
              margin="dense"
              id={"operation"}
              value={readingForm.values.operation}
              error={readingForm.touched.operation && readingForm.errors.operation}
              ...
              onBlur={readingForm.handleBlur}
              helperText={
                readingForm.touched.operation ? readingForm.errors.operation : ""
              }
            />
            <div style={{position: "absolute", top: "24px", left: "14px", width: "380px", overflowWrap: "break-word", lineHeight: "22px", fontSize: "15px", letterSpacing: "normal", fontFamily: "Arial, Helvetica, sans-serif"}}>
              {readingForm.values.operation !== "" ?
                readingForm.values.operation.split("").map((char,i, arr) => <span style={{color: checkChar(char, i)}} key={i}>
                        ...
                        <Fragment>
                          ...
                          {char == "{" ? <span style={{ fontWeight: "bold", display: 'flex', justifyContent: 'space-between' }}>Available Datapoints  <span style={{ cursor: 'pointer' }} onClick={() => { setShowHTML_Tooltip(false) }}>
                            ...
                            {props.datapoints.map((data) => {
                                const splitIndex = readingForm.values.operation.lastIndexOf("{")
                                if (i == splitIndex){
                                  return (<span style={{ cursor: 'pointer' }} onClick={() => { selectDatapointFromTheList(data) }}>{`${data.friendlyName} (${data.name})`}</span>)
                                }
                                const datapoint = readingForm.values.operation.slice(splitIndex+1)
        ...
        <TextField
          ...
          id="description"
          error={
            readingForm.touched.description && readingForm.errors.description
          }
          value={readingForm.values.description}
          onChange={readingForm.handleChange}
          onBlur={readingForm.handleBlur}
          helperText={
            readingForm.touched.description
              ? readingForm.errors.description
              : ""
          }
        />
        <span
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                checked={readingForm.values.defaultTimeseries}
                onChange={readingForm.handleChange}
              />
            }
          />
        ...
        >
          ...
          <FormControlLabel
            control={
              <Switch
                name="config"
                checked={readingForm.values.config}
                onChange={readingForm.handleChange}
              />
            }
          />
