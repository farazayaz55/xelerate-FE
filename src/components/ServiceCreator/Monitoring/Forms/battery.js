...
import { Zoom, Fade, InputAdornment } from "@mui/material";
...
export default function Battery(props) {
  ...
  const batteryForm = useFormik({
    initialValues: {
      description: "",
      datapoint: "",
      defaultCommand: "20",
      defaultName: "Battery Low",
      activeCommand: "10",
      activeName: "Battery Critically Low",
      name: "",
      charging: "",
      chargingStatus: "",
      dischargingStatus: "",
    ...
    onSubmit: async (values) => {
      ...
      let body = {
        ...
        metaData: {
          Minor: { Text: values.defaultName, Value: values.defaultCommand },
          Major: { Text: values.activeName, Value: values.activeCommand },
          chargingDatapoint: values.charging,
          chargingStatus: values.chargingStatus,
          dischargingStatus: values.dischargingStatus,
        },
  ...
  useEffect(() => {
    if(batteryForm.values.operation == ""){
      setErrors([])
    }
    const operationArray = batteryForm.values.operation.split("")
    const noOfOpenParanthesis = batteryForm.values.operation.split("{").length -1
    const noOfClosedParanthesis = batteryForm.values.operation.split("}").length -1
  ...
  const checkChar = (char, i, operation) => {
    const operationString = operation ? operation : batteryForm.values.operation
  ...
  const validateOperation = () => {
    ...
    const operationString = batteryForm.values.operation.replaceAll(",", ", ").replaceAll("(", "( ")
    ...
    let newString = batteryForm.values.operation
    ...
    if(!newErrors.length){
      if (newString !== batteryForm.values.operation){
        batteryForm.setFieldValue("operation", newString)
  ...
  const selectDatapointFromTheList = (datapoint)=>{
    let newInputValue = batteryForm.values.operation;
    ...
    batteryForm.setFieldValue('operation',newInputValue)
  }

  return (
    <div>
      <form onSubmit={batteryForm.handleSubmit}>
        <p
          style={{
            fontSize: "16px",
            color: "#666666",
        ...
        <TextField
          ...
          margin="dense"
          id="name"
          error={batteryForm.touched.name && batteryForm.errors.name}
          value={batteryForm.values.name}
          onChange={batteryForm.handleChange}
          onBlur={batteryForm.handleBlur}
          helperText={batteryForm.touched.name ? batteryForm.errors.name : ""}
        />
      {props.type == "datapoint" ? <TextField
          ...
          margin="dense"
          id={props.type}
          error={batteryForm.touched.datapoint && batteryForm.errors.datapoint}
          value={batteryForm.values.datapoint}
          onChange={batteryForm.handleChange}
          onBlur={batteryForm.handleBlur}
          helperText={
            batteryForm.touched.datapoint ? batteryForm.errors.datapoint : ""
          }
        />: 
        <FormTooltip
          open={infoPopup}
        ...
        >
          <div
            ...
            <TextField
              ...
              margin="dense"
              id={"operation"}
              value={batteryForm.values.operation}
              error={batteryForm.touched.operation && batteryForm.errors.operation}
              inputProps={
                {
                  style: {
              ...
              onBlur={batteryForm.handleBlur}
              helperText={
                batteryForm.touched.operation ? batteryForm.errors.operation : ""
              }
            />
            <div style={{position: "absolute", top: "24px", left: "14px", width: "380px", overflowWrap: "break-word", lineHeight: "22px", fontSize: "15px", letterSpacing: "normal", fontFamily: "Arial, Helvetica, sans-serif"}}>
              {batteryForm.values.operation !== "" ?
                batteryForm.values.operation.split("").map((char,i, arr) => <span style={{color: checkChar(char, i)}} key={i}>
                          ...
                          {char == "{" ? <span style={{ fontWeight: "bold", display: 'flex', justifyContent: 'space-between' }}>Available Datapoints  <span style={{ cursor: 'pointer' }} onClick={() => { setShowHTML_Tooltip(false) }}>
                            ...
                            <div style={{display: "flex", flexDirection:"column"}}>
                              {props.datapoints.map((data) => {
                                const splitIndex = batteryForm.values.operation.lastIndexOf("{")
                                if (i == splitIndex){
                                  return (<span style={{ cursor: 'pointer' }} onClick={() => { selectDatapointFromTheList(data) }}>{`${data.friendlyName} (${data.name})`}</span>)
                                }
                                const datapoint = batteryForm.values.operation.slice(splitIndex+1)
        ...
        <TextField
          ...
          id="description"
          error={
            batteryForm.touched.description && batteryForm.errors.description
          }
          value={batteryForm.values.description}
          onChange={batteryForm.handleChange}
          onBlur={batteryForm.handleBlur}
          helperText={
            batteryForm.touched.description
              ? batteryForm.errors.description
              : ""
          }
        />
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                checked={batteryForm.values.defaultTimeseries}
                onChange={batteryForm.handleChange}
              />
            }
          />
        ...
        >
          ...
          <FormControlLabel
            control={
              <Switch
                name="config"
                checked={batteryForm.values.config}
                onChange={batteryForm.handleChange}
              />
            }
          />
        ...
        >
          <b>Charging Status (Optional)</b>
        </p>

        <TextField
          label="Charging Datapoint"
          fullWidth
          margin="dense"
          id="charging"
          value={batteryForm.values.charging}
          onChange={batteryForm.handleChange}
        />
        <div style={{ display: "flex", gap: "20px" }}>
          <TextField
            ...
            margin="dense"
            id="chargingStatus"
            value={batteryForm.values.chargingStatus}
            onChange={batteryForm.handleChange}
          />
          <TextField
            label="Discharging Value"
            type="number"
            fullWidth
            margin="dense"
            id="dischargingStatus"
            value={batteryForm.values.dischargingStatus}
            onChange={batteryForm.handleChange}
          />
        </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: "20px" }}>
            <TextField
              ...
              id="defaultName"
              error={
                batteryForm.touched.defaultName &&
                batteryForm.errors.defaultName
              }
              value={batteryForm.values.defaultName}
              onChange={batteryForm.handleChange}
              onBlur={batteryForm.handleBlur}
              helperText={
                batteryForm.touched.defaultName
            ...
            <TextField
              ...
              id="defaultCommand"
              error={
                batteryForm.touched.defaultCommand &&
                batteryForm.errors.defaultCommand
              }
              value={batteryForm.values.defaultCommand}
              onChange={batteryForm.handleChange}
              onBlur={batteryForm.handleBlur}
              helperText={
                batteryForm.touched.defaultCommand
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: "20px" }}>
            <TextField
              ...
              id="activeName"
              error={
                batteryForm.touched.activeName && batteryForm.errors.activeName
              }
              value={batteryForm.values.activeName}
              onChange={batteryForm.handleChange}
              onBlur={batteryForm.handleBlur}
              helperText={
                batteryForm.touched.activeName
                  ? batteryForm.errors.activeName
                  : ""
              }
            />
            <TextField
              ...
              id="activeCommand"
              error={
                batteryForm.touched.activeCommand &&
                batteryForm.errors.activeCommand
              }
              value={batteryForm.values.activeCommand}
              onChange={batteryForm.handleChange}
              onBlur={batteryForm.handleBlur}
              helperText={
                batteryForm.touched.activeCommand
