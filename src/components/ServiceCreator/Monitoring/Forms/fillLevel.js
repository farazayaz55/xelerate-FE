...
export default function FillLevel(props) {
  ...
  useEffect(() => {
    if(fillLevelForm.values.operation == ""){
      setErrors([])
    }
    const operationArray = fillLevelForm.values.operation.split("")
    const noOfOpenParanthesis = fillLevelForm.values.operation.split("{").length -1
    const noOfClosedParanthesis = fillLevelForm.values.operation.split("}").length -1
  ...
  }, [fillLevelForm.values.operation])
  ...
  const checkChar = (char, i, operation) => {
    const operationString = operation ? operation : fillLevelForm.values.operation
  ...
  const validateOperation = () => {
    ...
    const operationString = fillLevelForm.values.operation.replaceAll(",", ", ").replaceAll("(", "( ")
    ...
    let newString = fillLevelForm.values.operation
    ...
    if(!newErrors.length){
      if (newString !== fillLevelForm.values.operation){
        fillLevelForm.setFieldValue("operation", newString)
  ...
  const selectDatapointFromTheList = (datapoint)=>{
    let newInputValue = fillLevelForm.values.operation;
    ...
    fillLevelForm.setFieldValue('operation',newInputValue)
  }
  
  return (
    <div>
      <form onSubmit={fillLevelForm.handleSubmit}>
        <p
          style={{
            fontSize: "16px",
            color: "#666666",
        ...
        <TextField
          ...
          margin="dense"
          id="name"
          error={fillLevelForm.touched.name && fillLevelForm.errors.name}
          value={fillLevelForm.values.name}
          onChange={fillLevelForm.handleChange}
          onBlur={fillLevelForm.handleBlur}
          helperText={
            fillLevelForm.touched.name ? fillLevelForm.errors.name : ""
          }
        />
      {props.type == "datapoint" ? <TextField
          ...
          margin="dense"
          id={props.type}
          error={fillLevelForm.touched.datapoint && fillLevelForm.errors.datapoint}
          value={fillLevelForm.values.datapoint}
          onChange={fillLevelForm.handleChange}
          onBlur={fillLevelForm.handleBlur}
          helperText={
            fillLevelForm.touched.datapoint ? fillLevelForm.errors.datapoint : ""
          }
        />: 
        <FormTooltip
          open={infoPopup}
        ...
        >
          <div
            ...
            <TextField
              ...
              margin="dense"
              id={"operation"}
              value={fillLevelForm.values.operation}
              error={fillLevelForm.touched.operation && fillLevelForm.errors.operation}
              ...
              onBlur={fillLevelForm.handleBlur}
              helperText={
                fillLevelForm.touched.operation ? fillLevelForm.errors.operation : ""
              }
            />
            <div style={{position: "absolute", top: "24px", left: "14px", width: "380px", overflowWrap: "break-word", lineHeight: "22px", fontSize: "15px", letterSpacing: "normal", fontFamily: "Arial, Helvetica, sans-serif"}}>
              {fillLevelForm.values.operation !== "" ?
                fillLevelForm.values.operation.split("").map((char,i, arr) => <span style={{color: checkChar(char, i)}} key={i}>
                        ...
                        <Fragment>
                          ...
                          {char == "{" ? <span style={{ fontWeight: "bold", display: 'flex', justifyContent: 'space-between' }}>Available Datapoints  <span style={{ cursor: 'pointer' }} onClick={() => { setShowHTML_Tooltip(false) }}>
                            ...
                            {props.datapoints.map((data) => {
                                const splitIndex = fillLevelForm.values.operation.lastIndexOf("{")
                                if (i == splitIndex){
                                  return (<span style={{ cursor: 'pointer' }} onClick={() => { selectDatapointFromTheList(data) }}>{`${data.friendlyName} (${data.name})`}</span>)
                                }
                                const datapoint = fillLevelForm.values.operation.slice(splitIndex+1)
        ...
        <TextField
          ...
          id="description"
          error={
            fillLevelForm.touched.description &&
            fillLevelForm.errors.description
          }
          value={fillLevelForm.values.description}
          onChange={fillLevelForm.handleChange}
          onBlur={fillLevelForm.handleBlur}
          helperText={
            fillLevelForm.touched.description
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                checked={fillLevelForm.values.defaultTimeseries}
                onChange={fillLevelForm.handleChange}
              />
            }
          />
        ...
        >
          ...
          <FormControlLabel
            control={
              <Switch
                name="config"
                checked={fillLevelForm.values.config}
                onChange={fillLevelForm.handleChange}
              />
            }
          />
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: "20px" }}>
            <TextField
              ...
              margin="dense"
              id="defaultName"
              value={fillLevelForm.values.defaultName}
              onChange={fillLevelForm.handleChange}
            />
            <TextField
              required
              ...
              margin="dense"
              id="defaultCommand"
              value={fillLevelForm.values.defaultCommand}
              onChange={fillLevelForm.handleChange}
            />
          </div>
        </div>
